...
// We use exactly the same input patterns (we just modify the output style in the replacements)
_defaultPatterns.xhtml = _defaultPatterns.html;

//
// Default block replacement functions, for css and js types
//
var defaultBlockReplacements = {
  css: function (block, superiorFileType) {
    var media = block.media ? ' media="' + block.media + '"' : '';
    var closeChar = (superiorFileType === 'xhtml') ? '/' : '';
    return '<link rel="stylesheet" href="' + block.dest + '"' + media + closeChar + '>';
  },
  js: function (block/*, superiorFileType*/) {
...
var FileProcessor = module.exports = function (superiorFileType, patterns, finder, logcb, blockReplacements) {
  if (!superiorFileType) {
    throw new Error('No type given');
  }
  this.superiorFileType = superiorFileType;

  if (!_.isArray(patterns)) {
    throw new Error('Patterns must be an array');
  }

  this.patterns = _defaultPatterns[superiorFileType] || [];
...
FileProcessor.prototype.replaceWith = function replaceWith(block) {
  ...
  if (typeof block.src === 'undefined' || block.src === null || block.src.length === 0) {
    ...
    result = block.indent + conditionalStart + this.blockReplacements[block.type](block, this.superiorFileType) + conditionalEnd;
  } else {
    result = '';
  }
  return result;
